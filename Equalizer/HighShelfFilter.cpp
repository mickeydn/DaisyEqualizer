...
void HighShelfFilter::Init() 
{
    for (int i = 0; i < NUM_HIGH_SHELF_COEFFS; i++) 
    {
        a[i] = 0;
        b[i] = 0;
    }
...
void HighShelfFilter::Init(float *a_coeffs, float *b_coeffs) 
{
    for (int i = 0; i < NUM_HIGH_SHELF_COEFFS; i++) 
    {
        a[i] = a_coeffs[i];
...
float HighShelfFilter::Process(float input)
{ 
...
void HighShelfFilter::MakeHighShelf(const float fs,
                                    const float fc,
                                    const float Q,
                                    const float A)
{
